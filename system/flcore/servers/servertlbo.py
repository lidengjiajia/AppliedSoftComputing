...
class FedTLBO(Server):
    ...
    def __init__(self, args, times):
        ...
        # 【新增】聚合策略配置（独立于检测方法）
        # 支持: fedavg, fedprox, scaffold, moon, fednova, tlbo
        self.aggregation_strategy = getattr(args, 'aggregation_strategy', 'fedavg')
    ...
    def _print_config(self):
        ...
        print(f"聚合策略: {self.aggregation_strategy}")
    ...
    def _fedact_aggregate(
        ...
        if normal_ids:
            ...
            # 【修改】根据聚合策略选择聚合方法
            aggregated = self._apply_aggregation_strategy(normal_grads, weight_list)
            ...
            return self._apply_aggregation_strategy(gradients, None), [], client_ids
    ...
    def _apply_aggregation_strategy(
        ...
        if strategy == 'tlbo':
            # TLBO优化聚合
            if hasattr(self, 'tlbo') and self.tlbo:
                return self.tlbo.aggregate(gradients, weights)
            # fallback到加权平均
            strategy = 'fedavg'
        ...
        if strategy == 'fedavg':
            ...
            # FedProx: 带近端项的加权平均
            # 注: 近端项主要在客户端本地训练中体现，服务端聚合同FedAvg
            if weights is None:
                weights = [1.0 / len(gradients)] * len(gradients)
            return sum(w * g for w, g in zip(weights, gradients))
        
        elif strategy == 'scaffold':
            # SCAFFOLD: 使用控制变量修正的聚合
            # 注: 完整SCAFFOLD需要维护控制变量，这里简化为加权平均
            if weights is None:
                weights = [1.0 / len(gradients)] * len(gradients)
            return sum(w * g for w, g in zip(weights, gradients))
            ...
            normalized = [g * (avg_norm / max(n, 1e-8)) for g, n in zip(gradients, norms)]
            if weights is None:
                weights = [1.0 / len(gradients)] * len(gradients)
            return sum(w * g for w, g in zip(weights, normalized))
